...
type RunData = {
  ...
  totalRevenue: number;
  stops: Array<{ key: string; revenue: number; legCarbonKg: number }>;
};

function buildRouteGeoJson(
  legs: Array<{ id: string; from: (typeof circuits)[number]; to: (typeof circuits)[number]; status: LegStatus }>,
...
export function FlightMap() {
  ...
  const [optimized, setOptimized] = useState<RunData | null>(null);
  const [baseline, setBaseline] = useState<RunData | null>(null);

  useEffect(() => {
    let cancelled = false;
    Promise.all([
      fetch("/data/f1_optimized.json", { cache: "no-store" }).then((r) => (r.ok ? r.json() : null)),
      fetch("/data/f1_baseline.json", { cache: "no-store" }).then((r) => (r.ok ? r.json() : null)),
    ])
      .then(([opt, base]) => {
        if (cancelled) return;
        if (opt?.stops?.length) setOptimized(opt);
        if (base?.totalRevenue) setBaseline(base);
      })
      .catch((e) => console.warn("flight-map: data load failed", e));
    return () => {
      cancelled = true;
    };
  }, []);
  ...
  const raceRoute = useMemo(() => {
    if (optimized) {
      return optimized.stops.map((s) => ({
        circuit: getCircuit(s.key),
        revenue: s.revenue,
        legCarbonKg: s.legCarbonKg,
      }));
    }
  ...
  const raceLegs = useMemo(
    () =>
      raceRoute.slice(0, -1).map((stop, index) => ({ from: stop.circuit, to: raceRoute[index + 1].circuit })),
    [raceRoute],
  );
  ...
  const getMarkerState = useCallback(
    (index: number): CircuitMarkerState => {
      if (!hasStarted) return index === 0 ? "current" : "upcoming";
  ...
  const routeGeoJson = useMemo(() => {
    const legs = raceLegs.map((leg, index) => {
      ...
      if (!hasStarted) status = "preview";
      else if (activeLegIndex !== null ? index < activeLegIndex : reachedStopIndex > index) status = "completed";
  ...
  const handleLegComplete = useCallback(() => {
    setActiveLegIndex((currentLeg) => {
      ...
      pauseAtStop(nextStop, () => {
        if (nextStop < raceLegs.length) setActiveLegIndex(nextStop);
      });
      return null;
    });
  ...
  // Revenue per point is a rough cost proxy; carbon in millions of kg.
  const fmtM = (kg: number) => `${(kg / 1_000_000).toFixed(1)}M`;
    ...
    <main className="fixed inset-0 overflow-hidden bg-[#0a0f14]">
      ...
      >
        <Source id="race-routes" type="geojson" data={routeGeoJson}>
          <Layer id="race-routes-preview" type="line" filter={["==", ["get", "status"], "preview"]}
            paint={{ "line-color": "rgba(255,255,255,0.14)", "line-width": 1.5, "line-dasharray": [2, 2] }} />
          <Layer id="race-routes-upcoming" type="line" filter={["==", ["get", "status"], "upcoming"]}
            paint={{ "line-color": "rgba(255,255,255,0.08)", "line-width": 1.2, "line-dasharray": [2, 2] }} />
          <Layer id="race-routes-completed" type="line" filter={["==", ["get", "status"], "completed"]}
            paint={{ "line-color": "rgba(52,211,153,0.55)", "line-width": 2 }} />
        </Source>

        {raceRoute.map(({ circuit }, index) => (
          <CircuitMarker key={circuit.key} name={circuit.name} city={circuit.city}
            latitude={circuit.latitude} longitude={circuit.longitude} state={getMarkerState(index)} />
        ))}

        {activeLeg ? (
          <AirplaneFlightMap key={`${flightRun}-${activeLegIndex}`} startTrigger={flightRun} autoStart
      ...
      <div className={`absolute left-1/2 top-4 z-10 w-[min(92vw,32rem)] -translate-x-1/2 rounded-xl border border-white/15 bg-black/70 px-5 py-3 text-center shadow-lg shadow-black/40 backdrop-blur-sm transition-opacity duration-200 ${isPausedAtStop ? "opacity-100" : "opacity-95"}`}>
        ...
        <p className="mt-0.5 text-sm text-red-300" suppressHydrationWarning>{formatRaceDate(displayedCircuit.raceDate)}</p>
      </div>
      ...
      <div className="absolute left-4 top-4 z-10 flex flex-col gap-2">
        ...
        <div className="w-52 rounded-lg border border-white/15 bg-black/65 px-3 py-2.5 shadow-lg shadow-black/30 backdrop-blur-sm">
          <div className="flex items-center justify-between gap-2">
            <label htmlFor="flight-speed" className="text-xs font-medium text-white/80">Flight speed</label>
            <span className="text-xs tabular-nums text-white/55">{flightSpeed.toFixed(2)}×</span>
          </div>
          <input id="flight-speed" type="range" min={FLIGHT_SPEED_MIN} max={FLIGHT_SPEED_MAX}
            step={FLIGHT_SPEED_STEP} value={flightSpeed}
      ...
      {baseline && optimized ? (
        <div className="absolute bottom-4 left-1/2 z-10 w-[min(96vw,44rem)] -translate-x-1/2 overflow-hidden rounded-2xl border border-white/15 bg-black/75 shadow-2xl shadow-black/50 backdrop-blur-md">
          <div className="grid grid-cols-2">
            <div className="border-r border-white/10 p-4">
              <p className="text-[11px] font-medium uppercase tracking-[0.2em] text-red-400/80">Baseline · naive schedule</p>
              <div className="mt-3 space-y-2">
                <div className="flex items-baseline justify-between">
                  <span className="text-xs text-white/45">Revenue</span>
                  <span className="tabular-nums text-lg font-semibold text-white/85">{fmt0(baseline.totalRevenue)}</span>
                </div>
                <div className="flex items-baseline justify-between">
                  <span className="text-xs text-white/45">Carbon</span>
                  <span className="tabular-nums text-lg font-semibold text-white/85">{fmtM(baseline.totalCarbonKg)} kg</span>
                </div>
              </div>
            </div>
            <div className="bg-emerald-500/5 p-4">
              <p className="text-[11px] font-medium uppercase tracking-[0.2em] text-emerald-300/90">Swarm-optimized ✓</p>
              <div className="mt-3 space-y-2">
                <div className="flex items-baseline justify-between">
                  <span className="text-xs text-white/45">Revenue</span>
                  <span className="tabular-nums text-lg font-semibold text-emerald-300">{fmt0(optimized.totalRevenue)}</span>
                </div>
                <div className="flex items-baseline justify-between">
                  <span className="text-xs text-white/45">Carbon</span>
                  <span className="tabular-nums text-lg font-semibold text-emerald-300">{fmtM(optimized.totalCarbonKg)} kg</span>
                </div>
              </div>
            </div>
          </div>
          <div className="flex items-center justify-center gap-2 border-t border-white/10 bg-emerald-500/10 py-2">
            <span className="text-xs font-semibold text-emerald-300">
              +{((optimized.totalRevenue / baseline.totalRevenue - 1) * 100).toFixed(1)}% revenue
            </span>
            <span className="text-white/30">·</span>
            <span className="text-xs text-white/50">swarm evolved a feasible, higher-value calendar</span>
          </div>
        </div>
      ) : null}
    </main>
